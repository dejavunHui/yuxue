package com.example.androidmvp.mvp.show.activity;
...
public class CreateRemarkActivity extends AppCompatActivity implements ImagePickerAdapter.OnRecyclerViewItemClickListener, BaseUpShowView {

    public static final int REMARKUP = 200;
    public static final int REMARKCANCEL = 201;
    ...
    private void initUpShow() {
        ...
        if(to != null)
            content.setHint("想对“"+to+"说点什么？");
        CreateRemarkActivity.RemarkUoClickListener listener = new CreateRemarkActivity.RemarkUoClickListener();
                    ...
                    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                        switch (position) {
                            case 0: // 直接调起相机
                                ...
                                Intent intent = new Intent(CreateRemarkActivity.this, ImageGridActivity.class);
                            ...
                            case 1:
                                //打开选择,本次允许选择的数量
                                ImagePicker.getInstance().setSelectLimit(maxImgCount - selImageList.size());
                                Intent intent1 = new Intent(CreateRemarkActivity.this, ImageGridActivity.class);
        ...
        public void onClick(View v) {
            switch (v.getId()){
                case R.id.true_send:
                    //发表pinglun
                    upRemark();
                    CreateRemarkActivity.this.setResult(REMARKUP);
                    CreateRemarkActivity.this.finish();
                    break;
                case R.id.cancel_send:
                    //暂时不加缓存
                    CreateRemarkActivity.this.setResult(REMARKCANCEL);
                    CreateRemarkActivity.this.finish();
                    break;
            }
        }

        public void upRemark(){
            String contentStr = content.getText().toString();
            presenter.upRemark(from,to,page,contentStr);
        }
    }
